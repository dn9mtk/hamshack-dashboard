...
export default function POTAPanel({ filter = FILTER_ALL, onFilterChange }) {
  ...
  if (err) return <PanelError message={err} onRetry={load} label="POTA" />;
    ...
    return (
      <div className="news-panel-content news-panel-slider">
        <div className="news-slider-card" style={{ minHeight: 60 }}>
          <div className="panel-empty">No POTA activators spotted. Try again later.</div>
        </div>
        <div className="contests-panel-footer">
          <a href={POTA_URL} target="_blank" rel="noopener noreferrer" className="contests-panel-link">
            Parks on the Air
          </a>
        </div>
      </div>
  ...
  const parkDisplay = item?.parkName || item?.reference || "—";
  const cardContent = item ? (
    <>
      ...
      <span className="news-slider-date">{parkDisplay}</span>
  ...
  return (
    <div className="news-panel-content news-panel-slider">
      <div className="pota-toggle contests-toggle" role="group" aria-label="Filter POTA by band">
      ...
      {hasMultiple && !emptyFilter && (
        <div
          ...
          role="group"
          aria-label="Navigate POTA activator"
          tabIndex={0}
        ...
        >
          <div className="news-slider-nav-row">
            <button
              ...
              onClick={() => go(-1)}
              aria-label="Previous POTA activator"
            >
              &#60;
            </button>
            ...
            <button
              ...
              className="news-slider-btn"
              onClick={() => go(1)}
              aria-label="Next POTA activator"
            >
              &#62;
            </button>
          </div>
